...
prepareboard(char *daboardfilename)
{
	...
	for(i=0;i<MAXPLAYERS;i++)
	{
		...
		clearbufbyte(&sync[i],sizeof(input),0L);
...
processinput(short snum)
{
	...
	if ((sync[snum].bits & 2048) > 0) {   // Shoot a bullet
		if ((numbombs[snum] == 0) && (((sync[snum].bits >> 13) & 7) == 2) && (myconnectindex == snum))
			locselectedgun = 0;
		if ((nummissiles[snum] == 0) && (((sync[snum].bits >> 13) & 7) == 3) && (myconnectindex == snum))
			locselectedgun = 1;
		if ((numgrabbers[snum] == 0) && (((sync[snum].bits >> 13) & 7) == 4) && (myconnectindex == snum))
			locselectedgun = 1;
...
domovethings()
{
	...
	for(i=connecthead;i>=0;i=connectpoint2[i])
		copybufbyte(&baksync[movefifoplc][i],&sync[i],sizeof(input));
	...
	if ((numplayers <= 2) && (recstat == 1))
	{
		...
		for(i=connecthead;i>=0;i=connectpoint2[i])
		{
			copybufbyte(&sync[i],&recsync[reccnt][j],sizeof(input));
			j++;
		}
...
loadgame()
{
	...
	kdfread(sync,sizeof(input),MAXPLAYERS,fil);
...
savegame()
{
	...
	dfwrite(sync,sizeof(input),MAXPLAYERS,fil);
